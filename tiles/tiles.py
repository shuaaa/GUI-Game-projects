...
class TileGame :
    def __init__ (self, sampleBoard="", dim=None) :
        if len(sampleBoard) == 9 or dim==3 :
             self.dim = 3;
             self.legalMoves = legalMoves3; self.goal="12345678_"
        elif len(sampleBoard) == 16 or dim==4 :
             self.dim = 4;
             self.legalMoves = legalMoves4; self.goal="123456789abcdef_"
        else : scream("Invalid dimension or sample board")
 
        self.makeManhatten()
        if sampleBoard and sorted(sampleBoard) != sorted(self.goal) :
            scream ("Invalid sample board")
